...
class BaseTable(tables.Table):
    ...
    @staticmethod
    def link_button(url, icon_class, btn_class="btn-outline-secondary", title=""):
        """A single generic icon-button link, for actions beyond edit/delete
        (e.g. "Reset Password" on the Users table). Also carries
        hx-boost="false" so it navigates normally even inside an
        HTMX-boosted table wrapper.
        """
        if not url:
            return ""
        return format_html(
            '<a href="{}" class="btn btn-sm {}" hx-boost="false" title="{}"><i class="bi {}"></i></a> ',
            url, btn_class, title, icon_class,
        )
