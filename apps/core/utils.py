...
def normalize_text(value):
    """Collapses all whitespace (including non-breaking spaces, which real
    spreadsheet exports commonly contain) to single regular spaces and
    strips the ends. None/NaN becomes "".
    """
    if value is None:
        return ""
...
def extract_pre_header_row_texts(df, header_row):
    """Returns a list of strings, one per non-blank row above `header_row`
    (1-indexed), from an already-loaded DataFrame (see `load_excel`).

    Every non-empty cell in a row is joined with a single space before
    being added to the list, so banner text that's split across several
    adjacent cells in the same row (a common source-file formatting quirk)
    still matches a single regex search against the whole row, rather
    than needing to land in one cell.
    """
    if header_row <= 1:
        return []
    texts = []
    for _, row in df.iloc[: header_row - 1].iterrows():
        cells = [normalize_text(v) for v in row if normalize_text(v) != ""]
        if cells:
...
def build_data_frame(df, header_row=1, start_col=1):
    ...
    for col in data.columns:
        if data[col].dtype == object:
            data[col] = data[col].apply(lambda v: " ".join(v.split()) if isinstance(v, str) else v)
